module MixedSemanticsProc where
...
-- Given envp, pname and (stm, envp'), return envp[ p |-> (stm, envp')]
updateEnvP :: EnvP -> Pname -> (Stm, EnvP) -> EnvP
updateEnvP (EnvP f) p (stm, envp) = EnvP $ \pname -> if pname == p
  then (stm, envp)
  else f pname

upd_p :: (DecP, EnvP) -> EnvP
upd_p ([], envp)                   = envp
upd_p ((pname, pbody):decps, envp) = upd_p (decps, updateEnvP envp pname (pbody, envp))
...
s_mixed stm s = s_ns stm _envp_init s
...
-- [call^rec_ns]
ns_stm (EnvP envp) (InterP (Call p) s) = FinalP s'
  where FinalP s'    = ns_stm (updateEnvP envp' p (stm, envp')) (InterP stm s)
        (stm, envp') = envp p
